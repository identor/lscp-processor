...
function createMatching(type) {
  var matching = {};
  switch (type) {
    case CallsProcessed.LS_SCORE:
      matching.scorer = { $exists: true };
      matching.processingEnd = { $exists: true };
      matching.callDuration = { $exists: true };
      matching.processingTimeSec = { $exists: true };
      break;
    case CallsProcessed.ELS_SCORE:
      matching.elsScorerName = { $exists: true };
      matching.elsProcessingEnd = { $exists: true };
      matching.callDuration = { $exists: true };
      matching.elsProcessingTimeSec = { $exists: true };
      break;
  }
  return matching;
}
...
function callsProcessed(collection, dateFrom, dateTo, cb) {
  ...
  var match = createMatching(CallsProcessed.LS_SCORE);
...
function enhancedCallsProcessed(collection, dateFrom, dateTo, cb) {
  var cb = arguments[arguments.length-1];
  var match = createMatching(CallsProcessed.ELS_SCORE);
  if (typeof dateFrom === 'function') {
    dateFrom = null;
  }
  if (typeof dateTo === 'function') {
